<?php
class ControllerCommonMeat extends Controller {
    public function index() {


        $this->document->setTitle("Buy Fresh and Clean Cut Meat online | Bargain.pk");
        ...
        if (isset($this->request->get['route'])) {
            $this->document->addLink($this->config->get('config_url').'gainz-meat', 'canonical');
        }
       ...
       // $filter['custom'] = '1201,1203,1206,1205,1237,1233,1214,1218,1257,1329';
        ...
        $latest_products =  $this->model_catalog_product->getLatestProductFromDepartment($did);
        $products = array();
        ...
        $this->response->setOutput($this->load->view('common/meat', $data));
    }
    ...
    public function SaveShipping(){
        ...
        $json = array();
        if($this->request->post['area'] && $this->request->post['city']){
            $this->session->data['meat_shipping_city'] = $this->request->post['city'];
            $this->session->data['meat_shipping_area'] = $this->request->post['area'];
            $json['error'] = false;
        } else {
            $json['error'] = ture;
        }
