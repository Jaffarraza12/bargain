...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
        $customerID = $this->getCustomerID($data['email']);
	    if($customerID){
            $data['guest'] = 0 ;
            $this->editCustomer($customerID, $data);
	...
	public function editCustomer($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', guest = '" . (int)$data['guest'] . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "' WHERE customer_id = '" . (int)$customer_id . "' ");
	}
	...
	public function getTotalCustomersByEmailNGuest($email) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "' AND `guest`=0");
	...
	public function CustomerCRMref($refno,$customer_id){
        $this->db->query("UPDATE " . DB_PREFIX . "customer SET `ref-no` = '" . $refno . "' WHERE customer_id = '" . (int)$customer_id . "' ");
