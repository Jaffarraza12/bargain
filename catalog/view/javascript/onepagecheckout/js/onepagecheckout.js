...
function LoadConfirmation(account_type){
	$.ajax({
		...
		success: function(html) { $('#onepagecheckout .confirm-order-content').html(html);}
	});
}
...
function placeAnOrder(payment){
    ...
    $.ajax({
        ...
        dataType: 'html',
        type: 'post',
        data: 'account_type=confirm',
        success: function(html) {
        	$('.confirm_payment_html').html(html)
        	if(payment == 'cod') {
                $.ajax({
                    url: 'index.php?route=extension/payment/' + payment + '/confirm',
                    dataType: 'json',
                    beforeSend: function () {
                        $('#button-confirm').button('Completing Order');
                    },
                    complete: function () {
                        $('#button-confirm').button('Completing Order');
                    },
