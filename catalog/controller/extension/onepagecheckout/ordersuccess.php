...
class ControllerExtensionOnepagecheckoutOrderSuccess extends Controller {
    ...
    public function index(){
        ...
        $guest = array();
        $guest['name'] = $order_info['firstname'];
        $guest['email'] = $order_info['email'];
        $guest['phone'] = $order_info['telephone'];
        $guest['shipping_city'] = $this->model_checkout_order->getCity($order_info['shipping_city']);
        $guest['shipping_state'] = $order_info['shipping_zone'];
        $guest['shipping_address'] = $order_info['shipping_address_1'];
        $guest['shipping_country'] = $order_info['shipping_country'];
        $guest['sts'] = time();
        $guest['mts'] = time();
        //$crm = $this->addAccount($guest);

        //FOR ADDING ORDER IN BARGIN ONE CRM
        //$crm = $this->model_checkout_order->getAccountCRM($order_info['customer_id']);
        //$this->addOrderInCRM($order_info,$crm);
		...
		if($order_info){
			...
			if($this->config->get('order_success_page_print_invoice')){
				$data['print_invoice']= $this->url->link('extension/onepagecheckout/ordersuccess/printinvoice','&order_id='.$order_info['order_id']);
			}
	...
	public function printinvoice(){
		...
		if($order_info){
				...
				$replace = array(
					...
					'city'      => $this->model_checkout_order->getCity($order_info['payment_city']),
				...
				$replace = array(
					...
					'city'      => $this->model_checkout_order->getCity($order_info['shipping_city']),
		...
		if($order_info){
				...
				foreach ($order_product_query->rows as $product) {
					...
					foreach ($order_option_query->rows as $option) {
						...
						$option_image = $this->model_checkout_order->getOptionImage($option['product_option_value_id']);
                        if($option_image){
                            $image = $this->model_tool_image->resize($option_image,$this->config->get('order_success_page_width'),$this->config->get('order_success_page_height'));
                        }
					}
    ...
    public function addOrderInCRM($order,$account_id){
        ...
        if($order['payment_code'] == 'cod'){
            $payment_code = 'COD';
        } else if($order['payment_code'] == 'jazzcash'){
            $payment_code = 'Due on Receipt';
        }
        $data = [
            'full_number' => $order['order_id'],
            'name' => $order['invoice_prefix'],
            'quote_stage' => 'Draft',
