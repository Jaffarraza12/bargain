<?php 
class ControllerProductQtyprice extends Controller {
	public function index() {
		...
		if ($product_info) {
            ...
            if (isset($this->request->post['quantity']) && $this->request->post['quantity']) {
                $data['price'] = $data['price'] * $this->request->post['quantity'];
                if ((float)$product_info['special']) {
                    $data['special'] =    $data['special']  * $this->request->post['quantity'];
                }
            }
                            ...
                            if (isset($this->request->post['option'][$option['product_option_id']]) && $this->request->post['option'][$option['product_option_id']] == $option_value['product_option_value_id']) {
                                if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
                                    ...
                                    if ($price) {
                                        ...
                                        $options_makeup = $options_makeup * $this->request->post['quantity'];
